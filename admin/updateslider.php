...
include_once('php_image_magician.php');
   ...
   if(move_uploaded_file($tempFile,$finalFileName))
   {
		...
		unlink($uniqfilename);
		    ...
		    $sql = mysql_query("update slider set title='".$title."',subtitle='".$subtitle."',description='".$description."',image='".$uniqfilename."',udate=now() where Id='".$sliderId."'") or die(mysql_error());
		        if($sql){
		            header("location:sliders.php?update=Succsess");
		        }

	}else{
	    
			$sql = mysql_query("update slider set title='".$title."',subtitle='".$subtitle."',description='".$description."',udate=now() where Id='".$sliderId."'") or die(mysql_error());
		        if($sql){
		            header("location:sliders.php?update=Succsess");
		        }
	}
