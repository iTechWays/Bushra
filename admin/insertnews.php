...
include_once('php_image_magician.php');
   ...
   if(move_uploaded_file($tempFile,$finalFileName))
   {
		...
		unlink($uniqfilename);
			...
			$sql=mysql_query("INSERT INTO news(title,description,pic,cdate)
			VALUES ('".$title."','".$description."','".$uniqfilename."',now())") or die(mysql_error());
		        if($sql){
		            header("location:news.php?insert=Succsess");
		        }
