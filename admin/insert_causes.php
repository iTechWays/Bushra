...
/*if(isset($_FILES["userfile"]["error"])){
   ...
   if(move_uploaded_file($tempFile,$finalFileName))
   {
  	...
  	$qur = mysql_query("SELECT * FROM causes WHERE title='".$title."'");
	
		if(mysql_num_rows($qur)>0){
			echo "errortitle";
			header("location:causes_insert.php?title=error");
		}
		else{ 
   
    // mysqli insert query
    $query = "INSERT into causes (title,description,image,cdate) VALUES ('$title','$desc','$uniqfilename','$sdate')";
    mysql_query($query);
    header("location: causeslist.php?ins=success");

		}
    }
}else{
    header("location: causes_insert.php?error=file");
}
?>
